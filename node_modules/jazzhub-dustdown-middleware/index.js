/*
 * Passes a markdown file at the given doc_path through a Dust compilation
 * pass (dust.compile(), dust.loadSource(), dust.render()).
 * Markdown fragments in the top-level of fragments_dir will be available as
 * Dust partials with naming {>"fragments_dir/filename"/}
 *
 * The Dust-processed markdown is attached to the request
 * as req.dusted_markdown
 */
...
var evaluateDustSyntax = function(doc_path, markdown_path, fragments_dir, callback) {
    fs.readFile(markdown_path, 'utf8', function(err, data) {
        ...
        dust.render(dustTemplateName, require(path.join(fragments_dir, 'phrases.json')), callback);
    });
};
...
module.exports = function(doc_path, fragments_dir) {
    ...
    return function(req, res, next) {
        ...
        evaluateDustSyntax(doc_path, markdown_path, fragments_dir, function(err, content) {
            if (err)
                return next();
