...
+ Create your own version of an IBM&reg; Bluemix&trade; DevOps Services project by cloning it in your space.
...
+ Create build and deployment jobs by using the Delivery Pipeline feature.
+ Deploy your app to IBM&reg; Bluemix&trade;.
...
1. Optional: [Explore the live version of the sample project you will work with][27]. The app analyzes Twitter users' collective sentiment about a search phrase. 
2. Optional: [Explore the contents of the Sentiment Analysis project][2]. This Node.js app uses a Git repository, and includes a Grunt build file.
3. Create your own copy of the Sentiment Analysis project by clicking the following button, which clones and deploys the project:
...
4. After the project is cloned and the deployment is complete, click **EDIT CODE** to continue.
...
1. Click **EDIT CODE**. The Web IDE opens and your project’s root directory is shown in the navigation pane. 
2. Click the `app.js` file to open it.
...
2. Select the check box next to `app.js`, and type a commit message. Somewhere in the message, be sure to include this phrase: `task <number of your work item>`. That phrase links this commit to the task. If you don’t remember the task number, return to the Track &amp; Plan area to find it.
![Ready to commit a change to app.js][13]
3. Click **COMMIT**. Your change is shown in the Active Branch pane.
...
When you clone a project by clicking the **Deploy to Bluemix** button, the IBM Continuous Delivery Pipeline for Bluemix configuration is created for your project automatically. In this section, you will explore how the preconfigured stages work.

The Delivery Pipeline for your Sentiment Analysis app has two stages: a build stage and a deployment stage. These stages form a pipeline. The build stage runs a build job on the included `Gruntfile.js` to validate your code. Then, the deployment stage runs a deploy job to deploy your code to Bluemix.

**Important:** When you use the Delivery Pipeline feature with Bluemix, you can generate charges to your Bluemix account. However, you can complete this tutorial for free because a project is granted 60 minutes of free build time per month. For more information about Bluemix, DevOps Services, and charges, [see Configure Bluemix billing for Bluemix DevOps Services][39].
...
With these connected stages, you have a fully functional pipeline. By default, the changes that you push to the master branch will trigger new builds. Successful builds are deployed according to your deployment stage configuration. 
...
2. After the deployment tile indicates that your app is running, click the URL that is under the app name. Make sure that your topic is in the INPUT section.
